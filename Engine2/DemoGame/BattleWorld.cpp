...
void BattleWorld::MakeUI() //앞에 경로는 다정해져있으니까 위에서 처리하자.. 
{
	...
	Fac->CreateButton(L"UI.png", []() {timeManager->SetTimeScale(0); }, { WinSizeX - 400, 200 }); //일시정시
	Fac->CreateButton(L"UI.png", []() {timeManager->SetTimeScale(2); }, { WinSizeX - 200, 200 }); //배속 //버튼도 이전에 크기정하는게 필요하지않을까나? 
